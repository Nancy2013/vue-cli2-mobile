...
var changeTime = (function() {
  ...
  var updateWeek = function(week, op) {
    //修改星期 week星期字符串  op 1 -1
  ...
  var timeEast8ToLocal = function(time) {
    //东八 -> 本地 读取时用  time 秒
    time = time + offsetEast8 - timezoneOffset();
    return time;
  };
  var timeLocalToEast8 = function(time) {
    //本地 -> 东八 设置时用   time 秒
    time = time - offsetEast8 + timezoneOffset();
    return time;
  };
  var timeToStamp = function(time) {
    ...
    return (
      (parseInt(arrTime[0]) * 60 + parseInt(arrTime[1])) * 60 +
      parseInt(arrTime[2])
    );
  };
  ...
  var changeTimerlist = function(data, op) {
    //单次时间 op 1 本地->东8  0 东8->本地
  ...
  var changePeriodlist = function(data, op) {
    //周期时间  op 1 本地->东8  0 东8->本地
  ...
  return function(dataOld, opType) {
    // dataOld 原始数据， opType 操作方式:1.设置定时的单个字符串（本地->东8）, 2.读取定时的长字符串
