/* eslint-disable */
import { dnaControl, deviceInfoPromise } from './call-native';
...
class Time {
  ...
  toString() {
    ...
    if (sun) {
      ...
      const sunPart = `${sun}${duration.seconds()}_${duration.minutes()}_${Math.floor(
        duration.asHours()
      )}`;
...
const call = async request => {
  ...
  const resp = await dnaControl(request, 'dev_subdev_timer');
  return resp.data;
};
